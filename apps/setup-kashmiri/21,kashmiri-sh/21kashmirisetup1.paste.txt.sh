...
# in osboxes.org user, paste the following from putty...

 sudo apt-get update ;  sudo apt-get install openssh-server wget 
 
 # can do next part from ssh, putty or mremoteNG - you can paste the rest with right click...
...
# no longer use osboxes.org user after this.


# use ssh to log into the system as albe user.

# use filezilla SFTP to copy the next files to /home/albe/shc and run them from there.
