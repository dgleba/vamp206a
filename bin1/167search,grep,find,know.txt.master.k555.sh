...
#find newest..
  ...
  find  . -mtime -1333 -type f -print0 | xargs -0 stat --printf='%.16y\t%s\t%n\n'  | sort -h -k1 |  grep -v '.git/' | grep -v tmp/ |grep -v x/ |grep -v datasys/ |grep -v sysdata/ |grep -v djangosite/static |grep -v venv |grep -v log/ |grep -v prg/  |grep -v pyc  | tail -n1054 
  # |grep gpt
